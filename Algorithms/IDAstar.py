...
class SequentialSearch(SearchBaseClass, ABC):
    ...
    def heuristic_function(self, node_current):
        ...
        # if obstacl[1]< 0:
        #     obstacle_info = (obstacl[0],obstacl[1]- obstacl[2]/2)
        #     wo = self.distance(node_current.get_position(),obstacle_info,1)
    ...
    def execute_search(self, time_pause) -> Tuple[Union[None, List[List[State]]], Union[None, List[MotionPrimitive]], Any]:
        ...
        print(self.get_obstacles_information(), "obs info \n")
        print(self.get_goal_information(), "goal info \n")
        print(self.get_node_information(node_initial) ,  'node info \n')
        """Enter your code here"""
        final_child,visited_nodes = self.IDAstar_search(node_current=node_initial)
    ...
    def IDAstar_search(self, node_current):
            
        thresh=self.evaluation_function(node_current)
        num_of_nodes=1 
        while True :
            temp,child_node,num_of_nodes = self.recursive_IDAstar(node_current,thresh,num_of_nodes )
            if temp < 0:
                return child_node,num_of_nodes
            elif temp=="inf":
                return False,num_of_nodes
            else :
                thresh= temp    

    def recursive_IDAstar(self, node_current,threshold,num_of_nodes):

        sum_cost = self.evaluation_function(node_current)
        if sum_cost > threshold:
            return sum_cost,node_current,num_of_nodes
        ...
        for primitive_successor in node_current.get_successors(): 
            ...
            if goal_flag:
                return -1,child,num_of_nodes
            
            temp_cost,child,num_of_nodes = self.recursive_IDAstar(node_current=child,threshold=threshold,num_of_nodes=num_of_nodes)
        ...
        return min_cost,child,num_of_nodes
